...
fun Home(onFinished: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            ...
            if (check != "1") {
                val paths = listOf("busybox", "01.tar.xz").map { name ->
                    val tmp = File(ctx.cacheDir, name)
                    ctx.assets.open(name)
                        .use { i -> FileOutputStream(tmp).use { o -> i.copyTo(o) } }
                    tmp.absolutePath
                }
                val script =
                    "mkdir -p $alsPath && cp ${paths[0]} ${alsPath}busybox && cp ${paths[1]} ${alsPath}01.tar.xz && chmod 755 ${alsPath}busybox && cd $alsPath && ./busybox tar -xJf 01.tar.xz && touch $markFile && rm ${
                        paths.joinToString(" ")
                    }"
