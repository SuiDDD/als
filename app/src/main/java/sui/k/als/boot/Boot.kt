...
fun BootScreen(onFinished: () -> Unit) {
    ...
    BackHandler(showTTY) { showTTY = false }
    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            ...
            if (!hasMark) {
                ...
                val script =
                    "mkdir -p $alsPath && cp ${tmpPaths[0]} ${alsPath}busybox && cp ${tmpPaths[1]} ${alsPath}01.tar.xz && chmod 755 ${alsPath}busybox && cd $alsPath && ./busybox tar -xJf 01.tar.xz && touch $markFile && rm ${
                        tmpPaths.joinToString(" ")
                    }"
    ...
    Box(
        ...
        Splash(header = {
            ...
            Column(modifier = Modifier.fillMaxSize()) {
                MenuLine(menuSys, font, Color.White) {
                    val count = sessions.count { it.first.contains(menuSys) } + 1
                    ...
                    sessions.add("$menuSys $count" to instance)
                    activeSession = instance
                    showTTY = true
                    scope.launch { delay(90); cmd(su); delay(90); cmd("clear && cd $alsPath && ./i") }
                }
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                    ...
                    LazyRow(
                        ...
                        items(sessions) { item ->
                            ...
                            Text(
                                ...
                                fontFamily = font,
                                modifier = Modifier.clickable(
                                    interactionSource = sSource, indication = null
                                ) {
                ...
                MenuLine(menuExit, font, Color.White) {
                    sessions.forEach { it.second.session.finishIfRunning() }
                    onFinished()
