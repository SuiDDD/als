...
@Composable
fun QvmImgResize(onPreview: (String) -> Unit, onExecute: (String) -> Unit) {
    ...
    Column {
        ...
        ALSList(data = stringResource(R.string.image_size), value = size, onValueChange = { size = it }, last = true)

        Spacer(Modifier.height(9.dp))

        ALSList(data = stringResource(R.string.preview), value = null, onClick = {
            if (path.isNotEmpty() && size.isNotEmpty()) onPreview("resize \"$path\" $size")
        })
        ALSList(data = stringResource(R.string.execute_action), value = null, onClick = {
            if (path.isNotEmpty() && size.isNotEmpty()) onExecute("resize \"$path\" $size")
        })
    }
}
