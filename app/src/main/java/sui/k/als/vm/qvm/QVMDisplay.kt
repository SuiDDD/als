...
fun QVMDisplay(state: MutableMap<String, Any>) {
    ...
    if (state["vnc_port"] == null) state["vnc_port"] = "9000"
    ...
    LaunchedEffect(vncPort, device, xres, yres) {
        state["display"] = "-device $device,xres=$xres,yres=$yres,disable-legacy=on,disable-modern=off -vnc 0.0.0.0:$vncPort "
    }
    ...
    Column {
        ...
        ALSList(data = stringResource(R.string.xres), value = xres, backgrounds = if (xres.isEmpty()) Color.Red else null, onValueChange = { state["xres"] = it })
