...
fun QVMCreate(config: QVMConfig? = null, onBack: () -> Unit) {
    ...
    LaunchedEffect(
        ...
        qvmMap["cdrom_cmd"] = buildString {
            cdroms.forEachIndexed { i, cd ->
                val path = cd["path"]?.toString()
                    ?: ""; if (path.isNotEmpty()) append("-drive file=\"$path\",if=none,id=dr_cd$i,format=raw,media=cdrom -device virtio-blk-pci,drive=dr_cd$i,bootindex=${cd["index"] ?: (i + 1)} ")
            }
        }
        qvmMap["disk_cmd"] = buildString {
            disks.forEachIndexed { i, disk ->
                val path = disk["path"]?.toString()
                    ?: ""; if (path.isNotEmpty()) append("-drive file=\"$path\",if=none,id=dr_d$i,cache=${disk["cache"] ?: "unsafe"} -device virtio-blk-pci,drive=dr_d$i,bootindex=${disk["index"] ?: (i + 2)} ")
            }
        }
        qvmMap["network_cmd"] =
            buildString { networks.forEachIndexed { i, net -> append("-netdev ${net["backend"] ?: "user"},id=net$i,hostfwd=${net["protocol"] ?: "tcp"}::${net["ports"] ?: "2222-:22"} -device ${net["device"] ?: "virtio-net-pci"},netdev=net$i ") } }
        val xres = qvmMap["xres"] ?: ""
        val yres = qvmMap["yres"] ?: ""
        val vncPort = qvmMap["vnc_port"]?.toString() ?: ":0"
        qvmMap["display_cmd"] =
            if (xres != "" && yres != "") "-device virtio-gpu-pci,xres=$xres,yres=$yres -vnc $vncPort " else "-device virtio-gpu-pci -vnc $vncPort "
        qvmMap["audio_cmd"] =
            if (qvmMap["audio"] == 1) "-audiodev aaudio,id=snd0 -device virtio-sound-pci,audiodev=snd0 " else ""
    }
    ...
    ExpressiveCanvas(
        ...
        onAction = {
            if (qvmMap["name"]?.toString()?.isBlank() != false) {
