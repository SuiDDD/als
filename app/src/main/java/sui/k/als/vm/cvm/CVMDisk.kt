...
fun CVMDisk(stateMap: MutableMap<String, Any>) {
    ...
    ALSList(
        ...
        value = stateMap["disk_path"]?.toString() ?: "",
        first = true,
        onValueChange = { stateMap["disk_path"] = it })
    ALSList(
        stringResource(R.string.firmware_path),
        value = stateMap["bios_path"]?.toString() ?: "$alsPath/app/crosvm/edk2-gunyah.fd",
        onValueChange = { stateMap["bios_path"] = it })
    ALSList(
        stringResource(R.string.enable_balloon), checked = stateMap["balloon"] == true, last = true
    ) { stateMap["balloon"] = stateMap["balloon"] != true }
}