...
@Composable
fun QvmImgAmend(onPreview: (String) -> Unit, onExecute: (String) -> Unit) {
    ...
    Column {
        ...
        ALSList(data = stringResource(R.string.preview), value = null, onClick = {
            if (path.isNotEmpty()) onPreview("amend -o \"$options\" \"$path\"")
        })
        ALSList(data = stringResource(R.string.execute_action), value = null, onClick = {
            if (path.isNotEmpty()) onExecute("amend -o \"$options\" \"$path\"")
        })
    }
}
