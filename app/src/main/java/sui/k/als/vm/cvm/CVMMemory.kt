...
fun CVMMemory(state: MutableMap<String, Any>) {
    ...
    ALSList(
        ...
        value = state["mem"]?.toString() ?: "2048",
        first = true,
        onValueChange = { state["mem"] = it })
    ALSList(
        stringResource(R.string.enable_hugepages), checked = state["hugepages"] == true
    ) { state["hugepages"] = state["hugepages"] != true }
    ALSList(
        stringResource(R.string.protected_vm), checked = state["protected"] == true, last = true
    ) { state["protected"] = state["protected"] != true }
}